...
async function insertRegistration(req) {
    ...
    const registrationId = statement['lastId']
...
async function mailRegistration(key) {
    const link = `https://answer.nickmatt.dev/confirm-email?key=${key}`
    await mail(
        req.body.email,
        'We Need You!',
